...
def remove_python_caches(root):
    """Remove generated bytecode that must never become trusted package content."""
    removed = 0
    ...
    for pyc in root.rglob("*.pyc"):
        if pyc.is_file():
            ...
            removed += 1
    if removed:
        print("[Velvet] Removed generated Python cache artifacts:", removed)
    return removed
...
def main(argv=None):
    ...
    if not args.skip_tests:
        ...
        run([python, "-m", "unittest", "discover", "-s", "tests"], cwd=root, env=test_env)
