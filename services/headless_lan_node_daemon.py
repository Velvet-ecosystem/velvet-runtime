...
This daemon combines the already-reviewed headless node identity/resource base,
specialist Ghost runner, authenticated Communications request/reply carrier, and
resource heartbeat client. It does not add a GUI, Court, generic remote command
surface, discovery protocol, shell, or hardware actuation.
...
from services.body_capacity import NodeResourceAdvertisement
...
class HeadlessLanNodeConfig:
    ...
    def __post_init__(self) -> None:
        ...
        for name, value in (("founder_host", self.founder_host), ("listen_host", self.listen_host)):
            if not isinstance(value, str) or not value.strip():
                raise HeadlessLanNodeConfigError("%s must be non-empty text" % name)
        for name, value in (("founder_port", self.founder_port), ("listen_port", self.listen_port)):
        ...
        for name, values in (
            ...
            ("temporary_absorption_capabilities", self.temporary_absorption_capabilities),
        ):
            _normalized_tuple(name, values, required=name in {"capabilities", "accepted_work_classes", "handlers"})
        if isinstance(self.max_concurrent_tasks, bool) or not isinstance(self.max_concurrent_tasks, int) or self.max_concurrent_tasks < 1:
            raise HeadlessLanNodeConfigError("max_concurrent_tasks must be a positive integer")
    ...
    def load(cls, path: Path) -> "HeadlessLanNodeConfig":
        ...
        if raw.get("schema") != HEADLESS_LAN_NODE_SCHEMA:
            raise HeadlessLanNodeConfigError("headless LAN node schema is unsupported")
        for key, expected in _TRANSPORT_FLAGS.items():
            if raw.get(key, expected) != expected:
                raise HeadlessLanNodeConfigError("configuration cannot change %s" % key)
        network = _mapping(raw, "network")
        role = _mapping(raw, "role")
        allowed_top = set((
            "schema", "node_config_path", "runner_state_path", "journal_path",
            "network", "role", "transport_only", "canonical", "grants_authority",
            "grants_execution", "grants_actuation", "authority",
        ))
        if set(raw) - allowed_top:
            raise HeadlessLanNodeConfigError("configuration contains unsupported top-level fields")
        allowed_network = set((
            "founder_peer_id", "founder_host", "founder_port", "listen_host",
            "listen_port", "peer_secret_file",
        ))
        if set(network) - allowed_network:
            raise HeadlessLanNodeConfigError("network configuration contains unsupported fields")
        allowed_role = set((
            "capabilities", "accepted_work_classes", "handlers", "max_concurrent_tasks",
            "refused_work_classes", "overflow_capable", "overflow_capabilities",
            "temporary_absorption_capabilities",
        ))
        if set(role) - allowed_role:
            raise HeadlessLanNodeConfigError("role configuration contains unsupported fields")
        return cls(
            ...
            founder_peer_id=_normalized("founder_peer_id", network.get("founder_peer_id")),
            ...
            capabilities=_normalized_tuple("capabilities", role.get("capabilities"), required=True),
            accepted_work_classes=_normalized_tuple(
                "accepted_work_classes", role.get("accepted_work_classes"), required=True
            ),
            handlers=_normalized_tuple("handlers", role.get("handlers"), required=True),
            max_concurrent_tasks=_positive_int(role.get("max_concurrent_tasks", 1), "max_concurrent_tasks"),
            refused_work_classes=_normalized_tuple(
                "refused_work_classes", role.get("refused_work_classes", ())
            ),
            overflow_capable=_boolean(role.get("overflow_capable", False), "overflow_capable"),
...
class HeadlessLanNodeDaemon:
    ...
    def __init__(self, config: HeadlessLanNodeConfig) -> None:
        ...
        profile = SpecialistNodeProfile(
            ...
            temporary_absorption_capabilities=config.temporary_absorption_capabilities,
    ...
    def _heartbeat_pair_once(self) -> None:
        now = time.time()
        # Local status is independent of Founder reachability.
        try:
            self.headless.observe_once(now=now)
        except Exception as exc:
            self._journal("local-status-failed", error_type=type(exc).__name__, error=str(exc))

        functional = self._functional_heartbeat_once(now=now)
        try:
            advertisement = self.headless.probe.probe(now=now)
    ...
    def _withdraw_resources(self) -> None:
        ...
        try:
            ...
            self._journal("resource-withdraw-failed", error_type=type(exc).__name__, error=str(exc))
            return
        self._journal(
            "resource-withdrawn",
...
def _load_json(path: Path) -> Mapping[str, Any]:
    ...
    try:
        if config_path.stat().st_size > _CONFIG_LIMIT_BYTES:
            raise HeadlessLanNodeConfigError("configuration exceeds bounded size limit")
...
def _normalized(name: str, value: Any) -> str:
    ...
    if text != normalized or len(text) > 128:
        raise HeadlessLanNodeConfigError("%s must be normalized text up to 128 characters" % name)
    return text


def _normalized_tuple(name: str, value: Any, required: bool = False) -> Tuple[str, ...]:
