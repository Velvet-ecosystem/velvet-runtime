...
class RuntimePreflightTests(unittest.TestCase):
    def make_env(self):
        ...
        return {name: str(path) for name, path in env.items()}
    ...
    def test_ready(self, _probe):
        with patch.dict(os.environ, self.make_env(), clear=False):
            report = run_runtime_preflight()
        self.assertTrue(report.ready)
        self.assertEqual(report.state, "ready")

    @patch("services.startup_doctor.importlib.util.find_spec", return_value=object())
    def test_short_signing_key_blocks(self, _probe):
        env = self.make_env()
        Path(env["VELVET_COURT_SIGNING_KEY_PATH"]).write_bytes(b"short")
